...
release_http() { api_absent "repos/$REPO/releases/tags/$TAG" && return 4; RC=$?; test "$RC" = 1 && return 0; return 1; }
...
preflight() {
  ...
  if release_http >/dev/null; then test "$ALLOW_RELEASE" = true || die "release already exists: $TAG"; else RC=$?; test "$RC" = 4 || die "release lookup failed"; fi
}
...
live_fields() { gh api "repos/$REPO/releases/tags/$TAG" --jq '[.id,.draft,.tag_name,.target_commitish,.name,(.body//""),(.prerelease|tostring),(.assets|length),(.assets[0].id//""),(.assets[0].name//""),(.assets[0].size//""),(.assets[0].digest//"")]|@tsv'; }
...
case "$OP" in
 ...
 prepare) test "$#" = 2 || die "usage: $0 prepare VERSION"; preflight; npm ci --ignore-scripts --no-audit --no-fund --prefix "$ROOT"; "$ROOT/scripts/test.sh"; "$ROOT/scripts/build-release.sh" "$VERSION"; "$ROOT/scripts/create-dmg.sh" "$VERSION"; E="$DMG.verification.evidence"; TMP="$MANIFEST.tmp.$$"; { printf 'schema=mobcrew-release-v1\nrepository=%s\nversion=%s\ntag=%s\ntarget_sha=%s\nartifact_name=%s\nartifact_size=%s\nartifact_sha256=%s\nbundle_id=%s\nbundle_version=%s\nbundle_build=%s\narchitectures=%s\nxcode_version=%s\nswift_version=%s\nnode_version=%s\npackage_lock_sha256=%s\nverification_evidence_sha256=%s\nverification_log_sha256=%s\n' "$REPO" "$VERSION" "$TAG" "$HEAD" "$(basename "$DMG")" "$(size "$DMG")" "$(sha "$DMG")" "$(kv "$E" bundle_id)" "$(kv "$E" version)" "$(kv "$E" build)" "$(kv "$E" architectures)" "$(xcodebuild -version | tr '\n' ' ')" "$(xcrun swift --version | head -1)" "$(node --version)" "$(sha "$ROOT/package-lock.json")" "$(sha "$E")" "$(sha "$DMG.verification.log")"; } >"$TMP"; validate_manifest "$TMP"; mv "$TMP" "$MANIFEST"; echo "$MANIFEST";;
 create-draft) test "$#" = 2 || die "usage: $0 create-draft VERSION"; preflight true; validate_manifest; test "$(kv "$MANIFEST" target_sha)" = "$HEAD" || die "manifest target is not current origin/main"; "$ROOT/scripts/verify-release-artifact.sh" "$DMG" "$VERSION" "$DIR/.draft-verification" >/dev/null; test "$(sha "$DIR/.draft-verification.evidence")" = "$(sha "$DMG.verification.evidence")" || die "material verification differs"; remote_tag_absent; if release_http >/dev/null; then :; else RC=$?; test "$RC" = 4 || die "release lookup failed"; gh release create "$TAG" --repo "$REPO" --draft --target "$(kv "$MANIFEST" target_sha)" --title "$TITLE" --notes "$BODY"; fi; IFS="$(printf '\t')" read -r RID DRAFT RTAG TARGET RTITLE RBODY PRE COUNT AID ANAME ASIZE DIGEST <<EOF
$(live_fields)
EOF
  positive "$RID"; test "$DRAFT|$RTAG|$TARGET|$RTITLE|$PRE" = "true|$TAG|$(kv "$MANIFEST" target_sha)|$TITLE|false" || die "conflicting draft metadata"; test "$(printf '%s\n' "$RBODY" | shasum -a 256 | awk '{print $1}')" = "$(printf '%s\n' "$BODY" | shasum -a 256 | awk '{print $1}')" || die "body mismatch"; case "$COUNT" in 0) gh release upload "$TAG" "$DMG" --repo "$REPO"; IFS="$(printf '\t')" read -r POST_ID POST_DRAFT POST_TAG POST_TARGET POST_TITLE POST_BODY POST_PRE POST_COUNT POST_AID POST_NAME POST_SIZE POST_DIGEST <<EOF
$(live_id_fields "$RID")
EOF
    test "$POST_ID|$POST_DRAFT|$POST_TAG|$POST_TARGET|$POST_TITLE|$POST_PRE|$POST_COUNT|$POST_NAME|$POST_SIZE" = "$RID|true|$TAG|$(kv "$MANIFEST" target_sha)|$TITLE|false|1|$(basename "$DMG")|$(size "$DMG")" || die "draft state changed during upload"; positive "$POST_AID" || die "invalid uploaded asset ID"; test "$(printf '%s\n' "$POST_BODY" | shasum -a 256 | awk '{print $1}')" = "$(printf '%s\n' "$BODY" | shasum -a 256 | awk '{print $1}')" || die "draft body changed during upload"; test -z "$POST_DIGEST" || test "$POST_DIGEST" = "sha256:$(sha "$DMG")" || die "uploaded API digest mismatch"; echo "draft upload matches; run verify-draft";; 1) positive "$AID" || die "invalid asset ID"; test "$ANAME" = "$(basename "$DMG")" && test "$ASIZE" = "$(size "$DMG")" || die "conflicting asset"; test -z "$DIGEST" || test "$DIGEST" = "sha256:$(sha "$DMG")" || die "conflicting asset digest"; echo "matching asset exists; run verify-draft";; *) die "extra assets present";; esac;;
 status) semver; context false; gh api "repos/$REPO/releases/tags/$TAG" --jq '{id,draft,tag_name,target_commitish,name,prerelease,assets:[.assets[]|{id,name,size,digest}]}' ;;
 verify-draft) semver; context false; validate_manifest; IFS="$(printf '\t')" read -r RID DRAFT RTAG TARGET RTITLE RBODY PRE COUNT AID ANAME ASIZE DIGEST <<EOF
$(live_fields)
EOF
  test "$DRAFT|$RTAG|$TARGET|$RTITLE|$PRE|$COUNT|$ANAME|$ASIZE" = "true|$TAG|$(kv "$MANIFEST" target_sha)|$TITLE|false|1|$(basename "$DMG")|$(size "$DMG")" || die "draft does not match"; positive "$RID"; positive "$AID"; T=$(mktemp -d); trap 'rm -rf "$T"' EXIT; gh release download "$TAG" --repo "$REPO" --pattern "$(basename "$DMG")" --dir "$T"; DOWN=$(sha "$T/$(basename "$DMG")"); test "$DOWN" = "$(sha "$DMG")" || die "download digest mismatch"; test -z "$DIGEST" || test "$DIGEST" = "sha256:$DOWN" || die "API digest mismatch"; TMP="$REMOTE_EVIDENCE.tmp.$$"; printf 'schema=mobcrew-remote-v1\nrepository=%s\nmanifest_sha256=%s\nrelease_id=%s\nasset_id=%s\ntag=%s\ntarget_sha=%s\ntitle=%s\nbody_sha256=%s\nasset_name=%s\nasset_size=%s\napi_digest=%s\ndownloaded_sha256=%s\n' "$REPO" "$(sha "$MANIFEST")" "$RID" "$AID" "$TAG" "$TARGET" "$TITLE" "$(printf '%s\n' "$RBODY" | shasum -a 256 | awk '{print $1}')" "$ANAME" "$ASIZE" "${DIGEST:-unavailable}" "$DOWN" >"$TMP"; validate_remote_evidence "$TMP"; mv "$TMP" "$REMOTE_EVIDENCE"; echo "$REMOTE_EVIDENCE";;
