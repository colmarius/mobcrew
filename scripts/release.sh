...
preflight() {
  semver; load_release_notes; command -v gh >/dev/null; command -v node >/dev/null; command -v xcodebuild >/dev/null; command -v xcrun >/dev/null; command -v base64 >/dev/null
  context true
  ...
  if RID=$(matching_draft_id); then :; else RC=$?; test "$RC" = 4 || die "draft lookup failed"; fi
}
...
live_id_fields() { gh api "repos/$REPO/releases/$1" --jq '[.id,.draft,.tag_name,.target_commitish,.name,((.body//"")|@base64),(.prerelease|tostring),(.assets|length),(.assets[0].id//""),(.assets[0].name//""),(.assets[0].size//""),(.assets[0].digest//"")]|@tsv'; }
case "$OP" in
...
$(live_id_fields "$RID")
EOF
  test "$LIVE_ID" = "$RID" || die "draft numeric identity changed"; test "$DRAFT|$RTAG|$TARGET|$RTITLE|$PRE|$COUNT|$ANAME|$ASIZE" = "true|$TAG|$(kv "$MANIFEST" target_sha)|$TITLE|false|1|$(basename "$DMG")|$(size "$DMG")" || die "draft does not match"; test "$RBODY" = "$(b64 "$BODY")" || die "body mismatch"; positive "$AID"; T=$(mktemp -d); trap 'rm -rf "$T"' EXIT; gh release download "$TAG" --repo "$REPO" --pattern "$(basename "$DMG")" --dir "$T"; DOWN=$(sha "$T/$(basename "$DMG")"); test "$DOWN" = "$(sha "$DMG")" || die "download digest mismatch"; test -z "$DIGEST" || test "$DIGEST" = "sha256:$DOWN" || die "API digest mismatch"; TMP="$REMOTE_EVIDENCE.tmp.$$"; printf 'schema=mobcrew-remote-v1\nrepository=%s\nmanifest_sha256=%s\nrelease_id=%s\nasset_id=%s\ntag=%s\ntarget_sha=%s\ntitle=%s\nbody_sha256=%s\nasset_name=%s\nasset_size=%s\napi_digest=%s\ndownloaded_sha256=%s\n' "$REPO" "$(sha "$MANIFEST")" "$RID" "$AID" "$TAG" "$TARGET" "$TITLE" "$(printf '%s\n' "$BODY" | shasum -a 256 | awk '{print $1}')" "$ANAME" "$ASIZE" "${DIGEST:-unavailable}" "$DOWN" >"$TMP"; validate_remote_evidence "$TMP"; mv "$TMP" "$REMOTE_EVIDENCE"; echo "$REMOTE_EVIDENCE";;
